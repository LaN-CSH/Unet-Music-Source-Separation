...
def readAud(S):  # audio = np.ndarray of loaded data, 9초 이상의 wav, 44100 파일
    ...
    zero_2ch = np.concatenate(
        (np.expand_dims(np.zeros(((S.shape[0] // sample_size) + 1) * sample_size - S.shape[0]), axis=-1)
    ...
    print(wave_batch_0.shape, '_ch0', end=' ')
...
def stft_read_batch(w0, w1):
    for k in range(w0.shape[0]):
        ...
        mag_0, phase_0 = librosa.magphase(stft_0)   # (1025, 256), (1025, 256)
        stft_0 = stft_0[:-1, :]
        stft_0 = np.expand_dims(stft_0, axis=-1)
        stft_0 = np.abs(np.expand_dims(stft_0, axis=0))
        phase_0 = np.expand_dims(phase_0, axis=0)  # (1, 1025, 256)
        # phase_0 = np.expand_dims(phase_0, axis=-1)  # (1, 1025, 256, 1)
        norm0 = stft_0.max()
        if stft_0.max() != 0:
            stft_0 /= stft_0.max()

        stft_1 = librosa.stft(w1[k], n_fft=window_size, hop_length=hop_length, center=False)
        mag_1, phase_1 = librosa.magphase(stft_1)   # (1025, 256), (1025, 256)
        stft_1 = stft_1[:-1, :]
        stft_1 = np.expand_dims(stft_1, axis=-1)  # (1024, 256, 1)
        stft_1 = np.abs(np.expand_dims(stft_1, axis=0))  # (1, 1024, 256, 1)
        phase_1 = np.expand_dims(phase_1, axis=0)  # (1, 1025, 256)
        # phase_1 = np.expand_dims(phase_1, axis=-1)  # (1, 1025, 256, 1)
        norm1 = stft_1.max()
        if stft_1.max() != 0:
            stft_1 /= stft_1.max()
    ...
    print('p0_shape:', p_0.shape, 'p1_shape:', p_1.shape)
    ...
    print(channel_0.shape, channel_1.shape, p_0.shape, p_1.shape)

    return channel_0, channel_1, p_0, p_1, norm0, norm1  # (?, 1024, 256, 1), (?, 1024, 256, 1), (?, 1024, 256), (?, 1024, 256)
...
def run():
    s, sr = librosa.load('/workspace/seungho/UNET_Sep/sample/wav_sample/y2mate.com - 349_EVGVJQtwxCY_320kbps.wav',
                         sr=44100)
    print(sr)
    wf0, wf1 = readAud(s)
    wb0, wb1, p_ch0, p_ch1, n0, n1 = stft_read_batch(wf0, wf1)
    ...
    with tf.Session() as sess:
        ...
        partial6_0 = sess.run(out, feed_dict={X: part6_0, train_mode: False})
        partial6_1 = sess.run(out, feed_dict={X: part6_1, train_mode: False})
        print(out0.shape, out1.shape)
        print(partial6_0.shape, partial6_1.shape)
        partial6_0 = partial6_0 * n0
        partial6_1 = partial6_1 * n1
...
def batch_istft(patch0, patch1, p_ch0, p_ch1):  # patch0,1 channel 0,1의 np.ndarray 파일
    s, sr = librosa.load('/workspace/seungho/UNET_Sep/sample/wav_sample/y2mate.com - 349_EVGVJQtwxCY_320kbps.wav',
                         sr=44100)
    print(sr, len(s))
    ...
    out0_drum = librosa.istft(output0[0, :, :, 0] * p_ch0[0, :, :, 0], win_length=window_size, hop_length=hop_length,
                              center=False)
    out0_bass = librosa.istft(output0[0, :, :, 1] * p_ch0[0, :, :, 0], win_length=window_size, hop_length=hop_length,
                              center=False)
    out0_other = librosa.istft(output0[0, :, :, 2] * p_ch0[0, :, :, 0], win_length=window_size, hop_length=hop_length,
                               center=False)
    out0_vocal = librosa.istft(output0[0, :, :, 3] * p_ch0[0, :, :, 0], win_length=window_size, hop_length=hop_length,
                               center=False)

    out1_drum = librosa.istft(output1[0, :, :, 0] * p_ch1[0, :, :, 0], win_length=window_size, hop_length=hop_length,
                              center=False)
    out1_bass = librosa.istft(output1[0, :, :, 1] * p_ch1[0, :, :, 0], win_length=window_size, hop_length=hop_length,
                              center=False)
    out1_other = librosa.istft(output1[0, :, :, 2] * p_ch1[0, :, :, 0], win_length=window_size, hop_length=hop_length,
                               center=False)
    out1_vocal = librosa.istft(output1[0, :, :, 3] * p_ch1[0, :, :, 0], win_length=window_size, hop_length=hop_length,
                               center=False)
    print('max:')
    print(out0_drum.shape, out0_drum.max())
    print(out0_bass.shape, out0_bass.max())
    print(out1_other.shape, out1_other.max())
    print(out1_vocal.shape, out1_vocal.max())
    ...
    print(np.mean(np.abs(drum)), np.mean(np.abs(bass)), np.mean(np.abs(other)), np.mean(np.abs(vocal)))
...
m1 = np.load('./sample/patch/stft_pt_phase0_part6.npy')
m2 = np.load('./sample/patch/stft_pt_phase1_part6.npy')
